...
Pertama, kolom tingkat resiko kehamilan harus diubah menjadi data numerik sehingga memudahkan untuk analisis. Perubahan tersebut mengikuti setting berikut:
low risk = 0
mid risk = 1
high risk = 2
...
Untuk menjawab Problem Statements kedua, maka dibangun model dengan outcomes yang diharapkan adalah 3 kategori: high risk, mid risk, low risk.
Data dibagi menjadi data train dan test dengan rasio 70:30 (dilakukan percobaan untuk rasio standar 80:20 kemudian dicek setelah dijalankan apakah memenuhi harapan atau perlu dilakukan penyesuaian untuk meningkatkan accuracy).

Model machine learning akan dibangun dengan beberapa algoritma dan dipilih model dengan akurasi tertinggi. Empat algoritma yang akan dilakukan adalah:
...
Data yang akan digunakan terdiri dari 1014 baris dan 7 kolom.
Kolom "RiskLevel" akan menjadi target (y) sedangkan kolom "Age", "SystolicBP", "DiastolicBP", "BS", "BodyTemp", dan "HeartRate" akan menjadi fitur penentu faktor resiko (X).
...
y adalah target yaitu tingkat resiko kehamilan dan X adalah fitur lain seperti 'Age', 'SystolicBP', 'DiastolicBP', 'BS', 'HeartRate', dan 'BodyTemp'
"""
...
"""### **Preparation 3: Train-Test-Split**

Membagi dataset menjadi data latih (train) dan data uji (test) dengan perbandingan 70:30. Ratio ini dianggap cukup karena besar data total yang adalah 1014. Selain itu, ratio ini memberikan nilai accuracy yang lebih baik pada data test, dikarenakan jumlah data test cukup besar dan model tidak overfitting.
"""
...
Akan tetapi, karena melibatkan banyak decision tree, random forest dapat membutuhkan waktu komputasi yang lebih lama dibandingkan dengan algoritma yang lebih sederhana, terutama untuk dataset yang besar. Model random forest dapat menjadi sangat kompleks, sehingga sulit untuk diinterpretasi secara mendalam.
Selain itu, parameter seperti jumlah pohon, kedalaman pohon, dan jumlah fitur yang dipilih secara acak perlu diset dengan tepat untuk mendapatkan hasil yang optimal.
...
"""- Membangun model terbaik berdasarkan Grid Search
- Melatih model dengan data training
- Membuat prediksi pada data train dan data test
"""
...
# Now you can use the best_model for further predictions
...
"""Keempat algoritma memberikan hasil yang sama.

## **Model Evaluation menggunakan metrik accuracy**
...
dengan kata lain metrik accuracy mengukur jumlah prediksi benar (baik positif maupun negatif) dibandingkan total prediksi.
Nilai 1 (100%) berarti model berhasil memprediksi seluruh data dengan benar, sedangkan nilai 0 (0%) berarti model gagal memprediksi seluruh data.
...
# Assuming 'accuracy' DataFrame has 'Model', 'Accuracy Train', and 'Accuracy Test' columns
# Melt the DataFrame to have a 'Set' column for 'Train' and 'Test'
accuracy_melted = pd.melt(accuracy, id_vars=['Model'], value_vars=['Accuracy Train', 'Accuracy Test'], var_name='Set', value_name='Accuracy')

# Create the bar plot using the melted DataFrame
sns.barplot(x='Model', y='Accuracy', hue='Set', data=accuracy_melted)
